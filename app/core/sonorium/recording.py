...
# Range: 3-30 minutes based on presence (low presence = longer gaps)
...
class RecordingThemeInstance:
    ...
    def get_stream(self, exclusion_coordinator: ExclusionGroupCoordinator = None, random_start: bool = False):
        ...
        if self.crossfade_enabled:
            base_stream = CrossfadeRecordingStream(self, random_start=random_start)
        else:
            base_stream = RecordingThemeStream(self, random_start=random_start)
...
class RecordingThemeStream:
    ...
    def __init__(self, instance: RecordingThemeInstance, random_start: bool = False):
        ...
        while True:
            try:
                ...
                if len(container.streams.audio) == 0:
                    logger.error(f'RecordingThemeStream: No audio stream in {self.instance.meta.path}')
                    # Yield silence for a bit then retry
                    for _ in range(100):
                        yield silence_chunk
                    time_module.sleep(1)
                    continue
                ...
                # Seek to random position on first loop if requested
                if first_loop and self.random_start and self.instance.meta.duration_seconds > 1.0:
                    # Seek to random position between 0% and 80% of track
                    max_seek = self.instance.meta.duration_seconds * 0.8
                    seek_time = random.uniform(0, max_seek)
                    # Convert to stream time base
                    seek_pts = int(seek_time / stream.time_base)
                    container.seek(seek_pts, stream=stream)
                    logger.debug(f'RecordingThemeStream: Random start at {seek_time:.1f}s for {self.instance.name}')
                    first_loop = False

                buffer = np.empty((1, 0), dtype=np.int16)
                error_count = 0  # Reset on successful open
                ...
                error_count += 1
                logger.error(f'RecordingThemeStream: Error playing {self.instance.name}: {e} (attempt {error_count}/{max_errors})')

                if error_count >= max_errors:
                    logger.error(f'RecordingThemeStream: Too many errors for {self.instance.name}, yielding silence indefinitely')
                    while True:
                        yield silence_chunk

                # Yield some silence while we retry
                for _ in range(50):
                    yield silence_chunk
...
class CrossfadeRecordingStream:
    ...
    def __init__(self, instance: RecordingThemeInstance, random_start: bool = False):
        self.instance = instance
        self.random_start = random_start
        self._first_decoder = True
        self.gen = self._gen()

    def _create_decoder(self, seek_random: bool = False):
        ...
        try:
            ...
            logger.error(f'CrossfadeRecordingStream: Failed to open {self.instance.meta.path}: {e}')
            # Return a generator that yields empty data
            def empty_decoder():
                return
                yield  # Never reached, but makes this a generator
            return empty_decoder()

        if len(container.streams.audio) == 0:
            logger.error(f'CrossfadeRecordingStream: No audio stream in {self.instance.meta.path}')
            container.close()
            def empty_decoder():
                return
                yield
        ...
        # Seek to random position if requested
        if seek_random and self.instance.meta.duration_seconds > 1.0:
            try:
                max_seek = self.instance.meta.duration_seconds * 0.8
                seek_time = random.uniform(0, max_seek)
                seek_pts = int(seek_time / stream.time_base)
                container.seek(seek_pts, stream=stream)
                logger.debug(f'CrossfadeRecordingStream: Random start at {seek_time:.1f}s for {self.instance.name}')
            except Exception as e:
                logger.warning(f'CrossfadeRecordingStream: Failed to seek in {self.instance.name}: {e}')
        ...
        def decode():
            try:
                ...
                logger.error(f'CrossfadeRecordingStream: Decode error in {self.instance.name}: {e}')
            finally:
                try:
                    container.close()
                except Exception:
    ...
    def _gen(self):
        ...
        while True:
            try:
                # Start first decoder - use random start if enabled
                current_decoder = self._create_decoder(seek_random=self.random_start and self._first_decoder)
                self._first_decoder = False
                next_decoder = None
                ...
                error_count = 0  # Reset on successful start
                ...
                error_count += 1
                logger.error(f'CrossfadeStream: Error in {self.instance.name}: {e} (attempt {error_count}/{max_errors})')

                if error_count >= max_errors:
                    logger.error(f'CrossfadeStream: Too many errors for {self.instance.name}, yielding silence indefinitely')
                    while True:
                        yield silence_chunk
    ...
    def _gen(self):
        ...
        def decode_file_once():
            ...
            try:
                ...
                logger.error(f'SparsePlaybackStream: Error decoding {self.instance.name}: {e}')
                return np.zeros(0, dtype=np.float32)
    ...
    def _gen(self):
        ...
        while True:
            ...
            try:
                ...
                logger.error(f'PresenceMixingStream: Error getting chunk from base stream: {e}')
                chunk = silence_chunk
