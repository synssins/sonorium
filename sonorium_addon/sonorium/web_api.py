...
def create_app(app_instance: 'SonoriumApp', channel_manager: ChannelManager | None = None) -> FastAPI:
    ...
    async def call_plugin_action(plugin_id: str, request: PluginActionRequest):
        ...
        # If plugin signals it created/modified themes, auto-refresh and include updated list
        if result.get('refresh_themes'):
            _app_instance.refresh_themes()
            # Build theme list in same format as GET /api/themes
            themes = []
            for theme in _app_instance.themes:
                track_count = len(theme.instances) if theme.instances else 0
                theme_data = {
